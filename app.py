...
with col_right:
    ...
    if st.session_state.last_audio:
        with result_slot.container():
            ...
            elif has_cloudinary_config():
                st.markdown(
                    "<div style='color:#334155;font-size:.78rem;margin-top:8px'>"
                    "☁️ Sẽ tự động upload lên Cloudinary sau khi tạo audio.</div>",
                    unsafe_allow_html=True,
                )
    else:
...
if generate_btn:
    with st.spinner("🎙️  Đang tổng hợp giọng nói…"):
        try:
            ...
            # DEBUG: hiển thị trạng thái secrets (xóa sau khi fix xong)
            try:
                import streamlit as _st
                _cld_debug = {
                    "CLD_CLOUD_NAME": bool(_st.secrets.get("CLD_CLOUD_NAME","")),
                    "CLD_API_KEY":    bool(_st.secrets.get("CLD_API_KEY","")),
                    "CLD_API_SECRET": bool(_st.secrets.get("CLD_API_SECRET","")),
                }
                st.info(f"🔍 Debug Secrets: {_cld_debug}")
            except Exception as _e:
                st.warning(f"🔍 Không đọc được Secrets: {_e}")
            ...
            if has_cloudinary_config():
                with st.spinner("☁️  Đang upload lên Cloudinary…"):
                    try:
                        ...
                        st.error(f"❌ Cloudinary lỗi không xác định: {type(cld_err).__name__}: {cld_err}")
            else:
                st.info("ℹ️ Chưa cấu hình Cloudinary Secrets — bỏ qua upload.")
