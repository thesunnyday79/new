...
from cloudinary_upload import upload_audio_to_cloudinary, has_cloudinary_config
...
.cld-badge-ok  { display:inline-flex;align-items:center;gap:6px;
                  background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.3);
                  color:#86efac;padding:3px 12px;border-radius:20px;font-size:.75rem;font-weight:600; }
.cld-badge-cfg { display:inline-flex;align-items:center;gap:6px;
                  background:rgba(251,191,36,.08);border:1px solid rgba(251,191,36,.25);
                  color:#fbbf24;padding:3px 12px;border-radius:20px;font-size:.75rem;font-weight:600; }
...
for k, v in {
    ...
    "fmt_choice": "WAV",
    "cld_result": None, "cld_message": None, "cld_message_type": None,
}.items():
...
with st.sidebar:
    ...
    st.markdown('<div class="sb-label">☁️ Cloudinary Upload</div>', unsafe_allow_html=True)
    if has_cloudinary_config():
        st.markdown('<span class="cld-badge-ok">● Cloudinary sẵn sàng</span>', unsafe_allow_html=True)
    else:
        st.markdown('<span class="cld-badge-cfg">● Chưa cấu hình</span>', unsafe_allow_html=True)
        st.caption("Thêm `CLD_CLOUD_NAME`, `CLD_API_KEY`, `CLD_API_SECRET` vào Secrets.")
...
# ── Hiển thị kết quả Cloudinary từ lần tạo trước (tồn tại qua rerun) ─────────
if st.session_state.get("cld_message"):
    msg  = st.session_state.cld_message
    mtype = st.session_state.cld_message_type
    if mtype == "success":
        lines = msg.split("\n", 1)
        st.success(lines[0])
        if len(lines) > 1 and lines[1]:
            ...
            st.link_button("🔗 Mở file trên Cloudinary", url, use_container_width=False)
    elif mtype == "warning":
        st.warning(msg)
    elif mtype == "error":
        st.error(msg)
    # Xóa sau khi hiển thị 1 lần
    st.session_state.cld_message      = None
    st.session_state.cld_message_type = None
...
with col_right:
    ...
    if st.session_state.last_audio:
        with result_slot.container():
            ...
            # Cloudinary result nếu đã upload
            if st.session_state.cld_result:
                r   = st.session_state.cld_result
                url = r.get("url", "")
                kb  = r.get("bytes", 0) // 1024
                fmt = r.get("format", "").upper()
                info_str = f"{kb} KB · {fmt}" if fmt else f"{kb} KB"
                st.markdown(
                    f"<div class='cld-card'>"
                    f"<div style='font-size:.72rem;font-weight:700;text-transform:uppercase;"
                    f"letter-spacing:.1em;color:#818cf8;margin-bottom:10px'>☁️ &nbsp;Cloudinary</div>"
                    f"<div class='cld-url-box'>{url}</div>"
                    f"<div style='display:flex;align-items:center;justify-content:space-between;margin-top:8px'>"
                    f"<a href='{url}' target='_blank' "
                    f"style='color:#a5b4fc;font-size:.82rem;text-decoration:none;font-weight:600'>"
                    f"🔗 Mở / Tải từ Cloudinary →</a>"
                    f"<span style='color:#475569;font-size:.72rem'>{info_str}</span>"
                    f"</div></div>",
                    unsafe_allow_html=True,
                )
                st.link_button("🔗 Mở link Cloudinary", url, use_container_width=True)
    else:
...
if generate_btn:
    with st.spinner("🎙️  Đang tổng hợp giọng nói…"):
        try:
            ...
            st.session_state.cld_result       = None
            ...
            with st.spinner("☁️  Đang upload lên Cloudinary…"):
                try:
                    try:
                        _cloud = st.secrets.get("CLD_CLOUD_NAME", "") or os.environ.get("CLD_CLOUD_NAME", "")
                        _key   = st.secrets.get("CLD_API_KEY",    "") or os.environ.get("CLD_API_KEY",    "")
                        _sec   = st.secrets.get("CLD_API_SECRET", "") or os.environ.get("CLD_API_SECRET", "")
                    except Exception:
                        _cloud = os.environ.get("CLD_CLOUD_NAME", "")
                        _key   = os.environ.get("CLD_API_KEY",    "")
                        _sec   = os.environ.get("CLD_API_SECRET", "")

                    if not all([_cloud, _key, _sec]):
                        st.session_state.cld_message      = "⚠️ Cloudinary chưa cấu hình — thiếu CLD_CLOUD_NAME / CLD_API_KEY / CLD_API_SECRET trong Secrets."
                        st.session_state.cld_message_type = "warning"
                    else:
                        fname  = f"tts_{selected_voice}_{int(time.time())}{audio_ext}"
                        result = upload_audio_to_cloudinary(audio_bytes, fname)
                        st.session_state.cld_result       = result
                        st.session_state.cld_message      = f"✅ Upload Cloudinary thành công!\n{result.get('url','')}"
                        st.session_state.cld_message_type = "success"

                except RuntimeError as cld_err:
                    st.session_state.cld_message      = f"❌ Cloudinary lỗi: {cld_err}"
                    st.session_state.cld_message_type = "error"
                except Exception as cld_err:
                    st.session_state.cld_message      = f"❌ Lỗi không xác định: {type(cld_err).__name__}: {cld_err}"
                    st.session_state.cld_message_type = "error" 
