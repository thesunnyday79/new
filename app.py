    ...
    with st.spinner("🎙️  Đang tổng hợp giọng nói…"):
        try:
            ...
            with st.spinner("☁️  Đang upload lên Cloudinary…"):
                try:
                    # Đọc credentials trực tiếp tại đây
                    try:
                    ...
                    if not all([_cloud, _key, _sec]):
                        st.warning(
                            "⚠️ Cloudinary chưa cấu hình. Vào **Settings → Secrets** thêm:\n\n"
                            "```toml\n"
                            "CLD_CLOUD_NAME = \"ten_cloud\"\n"
                            "CLD_API_KEY    = \"api_key\"\n"
                        ...
                        cld_url = result.get("url", "")
                        st.success(f"☁️ Upload thành công!")
                        st.code(cld_url, language=None)
                        st.toast("☁️ Đã upload lên Cloudinary!", icon="🔗")

                except RuntimeError as cld_err:
                    st.error(f"❌ Cloudinary lỗi: {cld_err}")
                except Exception as cld_err:
                    st.error(f"❌ Lỗi: {type(cld_err).__name__}: {cld_err}")
